...
def resting_heart_rate(heartrate, steps):
    ...
    # define RHR as the HR measurements recorded when there were zero steps taken during a
    # rolling time window of the preceding 12 minutes (including the current minute).
...
for line in f.split('\n'):
    ...
    if pat_id == 'AA0HAI1_Illness_1':
        ...
        filename = pat_id
    disease = tmp[1]
    symtom_date_raw = convert_time(tmp[2])
    diagnosis_date = convert_time(tmp[3])
    recovery_date = convert_time(tmp[4])
    time1 = symtom_date_raw + timedelta(days=0)
    time2 = recovery_date - timedelta(days=0)
    symtom_date = str(symtom_date_raw.year)+'-'+str(symtom_date_raw.month)+'-'+str(symtom_date_raw.day)
    ...
    if len(processed_data[time1:time2]) > 0:
        processed_data[['RHR']] = scaler.fit_transform(processed_data[['RHR']])
        covid_data = processed_data[time1:time2]
        Xs = []
        time_steps = 8
        for i in range(len(covid_data[['RHR']]) - time_steps):
            v = covid_data[['RHR']].iloc[i:(i + time_steps)].values
            Xs.append(v)
        Xs = np.array(Xs)
        if len(Xs) > 0:
            X_aug = augmentation(Xs)
            covid_data_final.append(Xs)
            covid_data_aug.append(X_aug)

covid_data_final = np.array(covid_data_final)
covid_data_aug = np.array(covid_data_aug)
print(covid_data_final.shape,covid_data_aug.shape)
np.save('covid.npy',covid_data_final)
np.save('covid_aug.npy',covid_data_aug)